...
![CI](https://github.com/gabkaclassic/klippa/actions/workflows/ci.yml/badge.svg)
...
навигацией клавиатурой и мышью, по Enter/клику вставляет выбранное в активное окно.
...
4. Стрелки/мышь/набор текста — выбор/фильтр. Enter или клик → `Promote(id)` +
   `GetContent(id)` → запись в буфер → закрытие popup → синтез `Ctrl+V` в прежнее
   окно (если включено). Клик вне меню закрывает его.
...
### deb-пакет (альтернатива install.sh)

Можно собрать `.deb` и ставить через apt — тогда зависимости подтянутся
автоматически, а удаление идёт через `apt remove`. Сборка ([nfpm](https://nfpm.goreleaser.com)):

```bash
NFPM=/path/to/nfpm ./packaging/build-deb.sh   # dist/klippa_<version>_all.deb
```

Версия берётся из `daemon/klippad/__init__.py` (`__version__`). Установка:

```bash
sudo apt install ./dist/klippa_<version>_all.deb   # apt подтянет python3-gi и пр.
klippa-enable                                       # один раз под своим юзером
```

Файлы кладутся системно: демон → `/usr/share/klippa/`, расширение →
`/usr/share/gnome-shell/extensions/klippa@local/`, юнит → `/usr/lib/systemd/user/`,
помощники → `/usr/bin/`. `postinst` (от root) только компилирует схему GSettings;
активация — **per-user** через `klippa-enable` (включает расширение в сессии и
стартует сервис), потом перелогин на Wayland. Управляющие команды (под своим юзером):

| Команда | Действие |
|---|---|
| `klippa-enable`  | включить расширение и сервис |
| `klippa-disable` | отключить расширение и остановить сервис (данные не трогает) |
| `klippa-config`  | открыть GUI настроек (= `gnome-extensions prefs klippa@local`) |

Удаление: `sudo apt remove klippa` (данные в `~/.config/klippa` и `~/.local/share`
сохраняются). На релизах GitHub `.deb` собирается и прикрепляется автоматически.
...
packaging/   klippad.service, install.sh, uninstall.sh, gnome-upgrade.sh,
             ...
             nfpm.yaml + build-deb.sh (deb), deb/ (системный юнит,
             klippa-enable/disable/config, postinst/postrm)
...
**CI** ([`.github/workflows`](../.github/workflows)): `ci.yml` гоняет весь pytest на
каждый push и PR (ставит те же системные пакеты, что и `install.sh`); `release.yml`
при создании релиза собирает zip-бандл (`pack.sh`, фолбэк на `zip` без gnome-shell)
и deb-пакет (`build-deb.sh` через nfpm) и прикрепляет оба к релизу.
...
- **Окно открывается, но всегда пустое** — демон не на шине. Частый случай: БД
  зашифрована старым ключом keyring (после `--purge`/смены профиля). Начиная с
  0.1.1 демон сам откладывает нечитаемый файл в `history.db.unreadable-<время>` и
  стартует с пустой историей; в логе — строка про `HistoryUnreadable`. Старые
  `.unreadable-*` можно удалить, если ключ к ним уже не вернётся.
